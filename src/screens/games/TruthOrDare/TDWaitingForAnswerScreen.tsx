...
type Props = NativeStackScreenProps<any, 'TDWaitingForAnswerScreen'>;

const TDWaitingForAnswerScreen: React.FC<Props> = ({ navigation, route }) => {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        ...
        if (res.data.state === 'review') {
          navigation.replace('TDReviewScreen', {
            roomId,
            playerId,
            question: res.data.truthQuestion,
            answer: res.data.truthAnswer,
            round: res.data.currentRound,
...
export default TDWaitingForAnswerScreen;
